...
"""
        ...
        # Fallback model cascade to ensure zero-downtime if primary model exceeds free-tier quota
        models_to_try = [
        ...
        for model_name in unique_models:
            try:
                logger.info(f"Generating questions using Gemini model: {model_name} (temperature 0.0)")
                ...
                if validated.questions:
                    logger.info(f"Successfully generated {len(validated.questions)} MCQs with model {model_name}")
                    return validated.questions
            except Exception as err:
                last_error = err
                logger.warning(f"Generation attempt with model '{model_name}' failed: {err}. Trying next candidate if available.")
                continue
        ...
        raise RuntimeError("No questions were generated by AI engine.")
