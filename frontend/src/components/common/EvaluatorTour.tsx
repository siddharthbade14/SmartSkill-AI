...
export const EvaluatorTour: React.FC<EvaluatorTourProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/75 backdrop-blur-md animate-in fade-in duration-200">
      <div className="relative w-full max-w-2xl bg-gradient-to-br from-[#0B132B] via-[#0F172A] to-[#1E293B] border border-blue-500/40 rounded-3xl shadow-2xl overflow-hidden text-white">
        
        {/* National Emblem & Top Accent Strip */}
        <div className="h-2 bg-gradient-to-r from-amber-500 via-[#134E4A] to-blue-500" />
        ...
        <div className="p-6 sm:p-8">
          ...
          <div className="flex items-start justify-between gap-4 mb-4">
            <div className="flex items-center gap-3">
              <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-blue-500/30 to-indigo-600/30 border border-blue-400/40 flex items-center justify-center text-amber-300 shadow-md">
                <IconComponent className="w-6 h-6 animate-pulse" />
              </div>
              <div>
                <span className={`inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-[11px] font-bold border uppercase tracking-wider ${currentStep.badgeColor}`}>
            ...
            <button
              onClick={onClose}
              className="p-1.5 rounded-xl bg-white/10 hover:bg-white/20 border border-white/20 text-slate-300 hover:text-white transition shadow-xs"
          ...
          {/* Description */}
          <p className="text-xs sm:text-sm text-slate-300 leading-relaxed mb-5 bg-white/5 p-3.5 rounded-xl border border-white/10">
          ...
          <div className="flex items-center justify-between pt-4 border-t border-slate-700/60">
            ...
            <div className="flex items-center gap-2">
              {!isFirstStep && (
                <button
                  onClick={handlePrev}
                  className="inline-flex items-center gap-1 px-4 py-2 rounded-xl bg-white/10 hover:bg-white/20 border border-white/20 text-xs font-bold text-white transition shadow-xs active:scale-95"
                >
              ...
              <button
                onClick={handleNext}
                className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 text-xs sm:text-sm font-bold text-white shadow-lg shadow-blue-600/30 border border-blue-400/40 transition active:scale-95"
              >
